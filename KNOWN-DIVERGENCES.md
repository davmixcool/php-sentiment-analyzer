...
It is a major release because the fixes move roughly 45% of scores, and it
requires **PHP 8.1+**. **This 1.x line keeps the behaviour documented below**,
unchanged and maintained, for users on older runtimes. Nothing here will be fixed
on 1.x — the fix is the upgrade, and it is only available to those who can move
PHP version.
